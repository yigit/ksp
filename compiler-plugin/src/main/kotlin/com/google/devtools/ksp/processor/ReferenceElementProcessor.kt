...
open class ReferenceElementProcessor : AbstractTestProcessor() {
    ...
    override fun process(resolver: Resolver) {
        ...
        for (i in sortedReferences)
            results.add(
                "KSClassifierReferenceImpl: Qualifier of ${i.element}" +
                    " is ${(i.element as KSClassifierReference).qualifier}"
            )

        val descriptorReferences =
            sortedReferences.map { KSTypeReferenceDescriptorImpl.getCached((it.resolve() as KSTypeImpl).kotlinType) }
        for (i in descriptorReferences) {
            results.add(
                "KSClassifierReferenceDescriptorImpl: Qualifier of ${i.element}" +
                    " is ${(i.element as KSClassifierReference).qualifier}"
            )
        }
        ...
        for (i in javaReferences) {
            results.add(
                "KSClassifierReferenceJavaImpl: Qualifier of ${i.element}" +
                    " is ${(i.element as KSClassifierReference).qualifier}"
            )
        }
    }
